...
def save_into_db(rcd) :
    ...
    mysql = 'insert into twitterscraper_merged_new2_nlp2(myindex,mydatetime,mytext,sentiment,magnitude,classify_name,classify_confidence,processdatetime,processind) values("%s","%s","%s","%s","%s","%s","%s","%s","%s")' \
        %(myrcd[0],
...
cursor.execute('select * from twitterscraper_merged_new2 where myindex not in (select myindex from twitterscraper_merged_new2_nlp2)')
