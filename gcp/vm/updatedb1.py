...
def save_into_db(rcd) :
    ...
    mysql = 'insert into twitterscraper_merged_new_nlp(myindex,mydatetime,mytext,sentiment,magnitude,classify_name,classify_confidence,processdatetime,processind) values("%s","%s","%s","%s","%s","%s","%s","%s","%s")' \
        %(myrcd[0],
...
db = pymysql.connect(host='35.234.25.100',
                             port=3306,
                             user='neskwoe',
                             password='nesk1234',
                             db='newsMySQLDatabase',
                             charset='utf8')
cursor =db.cursor()
#create_db()
...
cursor.execute('select * from twitterscraper_merged_new where myindex not in (select myindex from twitterscraper_merged_new_nlp)')
